...
migrate((app) => {
  ...
  collection.createRule = "patient_id = @request.auth.id";
  collection.updateRule = "patient_id = @request.auth.id";
  ...
  collection.listRule = "clinic_id = @request.auth.clinic_id || patient_id = @request.auth.id";
  collection.viewRule = "clinic_id = @request.auth.clinic_id || patient_id = @request.auth.id";
  collection.createRule = "patient_id = @request.auth.id || clinic_id = @request.auth.clinic_id";
  collection.updateRule = "patient_id = @request.auth.id || clinic_id = @request.auth.clinic_id";
  collection.deleteRule = "clinic_id = @request.auth.clinic_id";
  return app.save(collection);
  } catch (e) {
